...
*/
function buildResetArgs(mode, hash) {
	return [
		"reset",
		`--${mode}`,
		hash
...
*/
async function currentBranchOf(root, runGit) {
	try {
...
async function scanRepos(cwd, runGit) {
		...
		if (top !== "") {
			...
			if (!seen.has(root)) {
				...
				entries.push({
					...
					name: basename(root),
					depth: 0,
					branch: await currentBranchOf(root, runGit)
				});
			}
		}
	} catch {}
	...
	for (const entry of level1Repo) if (entry !== null) {
	...
	for (const entry of level2Repo) if (entry !== null) {
		const { root, branch } = entry;
		if (!seen.has(root)) {
			seen.add(root);
			entries.push({
...
async function discoverRepo(dir, runGit) {
...
async function endpointRef(ctx, payload) {
	...
	if (action === "reset") {
		if (hash === void 0) throw new WbError("bad-request", "reset requires hash");
	...
	if (action === "revert") {
		if (hash === void 0) throw new WbError("bad-request", "revert requires hash");
	...
	if (action === "merge") {
		if (name === void 0) throw new WbError("bad-request", "merge requires a branch name");
...
function resetModeOf(raw) {
	const mode = raw?.["mode"];
	if (mode === void 0 || mode === "" || mode === "mixed") return "mixed";
...
async function endpointWorktreeContent(ctx, payload) {
	const sessionId = stringOrUndefined(payload, "sessionId");
	...
	const slice = (text) => {
		if (text.length <= MAX_FILE_CONTENT_CHARS) return {
			content: text,
			truncated: false
		};
	...
	let old = blank;
	try {
		const stdout = await runGit(root, buildShowFileArgs("HEAD", filePath), 4194304);
		if (stdout.includes("\0")) old = {
			content: "",
			exists: true,
			truncated: false,
			binary: true
	...
	if (!rel.startsWith("..") && !isAbsolute(rel)) try {
		if ((await stat(abs)).isFile()) {
			const buf = await readFile(abs);
			if (buf.includes(0)) next = {
				content: "",
				exists: true,
				truncated: false,
				binary: true
	...
	ctx.effect(() => ctx.webServer.register({
		...
		handler: async (req, res) => {
			...
			try {
				...
				else if (method === "worktree-content") value = await endpointWorktreeContent(ctx, payload);
				else {
					writeError(res, new WbError("not-found", `unknown /wb-git method "${method}"`, 404));
					return;
				}
...
//#endregion
export { MAX_SCAN_DIRS, SEP, WbError, apply, buildBranchArgs, buildCheckoutArgs, buildCommitArgs, buildConfigGetArgs, buildConfigSetArgs, buildCreateBranchArgs, buildCreateTagArgs, buildDeleteBranchArgs, buildDeleteTagArgs, buildDetailArgs, buildFetchArgs, buildFetchIntoArgs, buildLogArgs, buildMergeArgs, buildPullArgs, buildPushBranchArgs, buildPushTagArgs, buildRemoteAddArgs, buildRemoteListArgs, buildRemoteRemoveArgs, buildRemoteSetUrlArgs, buildRenameBranchArgs, buildResetArgs, buildRevertArgs, buildShowFileArgs, buildShowRefArgs, buildStageAddArgs, buildStageResetArgs, buildStatusFilesArgs, buildSwitchArgs, buildSwitchDetachArgs, currentBranchOf, inject, isRepoRoot, name, parseBranches, parseCommitDetail, parseGitLog, parseNameStatus, parseNumStat, parseRemotes, parseShowRef, parseStatusFiles, runGit, scanRepos };
